...
#####################################################################################################
### CONFIG VARS #####################################################################################
declare LLTEST_CMD="/app/svends_run -num_edicts 3072 -norestart +sv_lan 1";
declare LLTEST_NAME="gamesvr-svencoop-$(date '+%H%M%S')";
#####################################################################################################
#####################################################################################################
...
should_lack 'FATAL ERROR (shutting down): Unable to initialize Steam.' 'Server is not having fatal issues with Steam'
should_lack "Invalid game type '" 'Server is not having issues finding game data'
should_have 'Setting breakpad minidump AppID = 225840' 'Sever started executing';
...
should_lack 'server.so: libssl.so.1.0.0: cannot open shared object file' 'Server is not crashing';

# Verify server responds to commands
should_echo "sv_cheats" '"sv_cheats" is "0"';
#####################################################################################################
#####################################################################################################
